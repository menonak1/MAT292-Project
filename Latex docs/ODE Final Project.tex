...
\usepackage[draft]{graphicx} % For inserting figures (remove [draft] once plots are saved)
...
underlying ODEs without the added complexity of full 3D trajectory tracking.
...
matrix $M^{-1}$ described below. In the simulations we choose simple step
...
To stabilize the unstable dynamics, we implement four independent controllers
...
$(\phi, \theta, \psi)$ we ultimately set $K_i \approx 0$ and use a PD
structure, while the altitude controller retains a nonzero $K_i$ to remove
...
\end{align*}
This requires the inverse mapping $\mathbf{T}_m = M^{-1} \mathbf{u}_v$, where $M^{-1}$ is:
$$
M^{-1} =
\begin{bmatrix}
1/4 & -1/(2L) & 0 & -1/(4k_m) \\
...
method to solve the 12 ODEs~\cite{ascher1998computer}. We use a main loop that calls
...
\caption{Attitude (roll, pitch, yaw) response to simultaneous step commands (Roll=10$^{\circ}$, Yaw=30$^{\circ}$). The PD controller achieves fast rise time with minimal overshoot, and the Pitch axis remains stable at $0^{\circ}$, demonstrating excellent cross-coupling rejection.}
\label{fig:attitude}
\end{figure}
...
def quadcopter_dynamics(t, x, m, g, I, L, k_m, T1, T2, T3, T4):
    ...
    # Torques in Body frame (based on final invertible M_inv)
    tau_phi   = L * (-T1 + T3)
    tau_theta = L * (-T2 + T4)
    ...
    # 1. Translational Position Rate:
    dot_x[0:3] = vel
    
    # 2. Angular Angle Rate:
    W = np.array([
...
if __name__ == "__main__":
    ...
    # --- Initialize Controllers (FINAL TUNED VALUES) ---
    # Altitude Controller (PID)
    pid_z = PIDController(Kp=0.17, Ki=0.21, Kd=1.0)      
    
    # Attitude Controllers (PD/P, Symmetry used)
    pid_roll = PIDController(Kp=0.08, Ki=0.001, Kd=0.043)   
    pid_pitch = PIDController(Kp=0.08, Ki=0.000, Kd=0.043)  
    pid_yaw = PIDController(Kp=0.8, Ki=0.000, Kd=0.16)    
    ...
    for i in range(1, num_steps):
        ...
        # Commands: Roll 10 degrees & Yaw 30 degrees (t=2s to t=8s)
        if 2.0 < t_start < 8.0:
    ...
    # Plot Roll and Yaw desired targets
    # Scale xmin/xmax relative to total_time=15.0
    plt.axhline(y=10.0, xmin=2.0/15.0, xmax=8.0/15.0, color='r', linestyle=':', label='Roll command')
    plt.axhline(y=30.0, xmin=2.0/15.0, xmax=8.0/15.0, color='g', linestyle=':', label='Yaw command')
